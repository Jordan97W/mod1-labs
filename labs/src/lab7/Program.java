package lab7;

public class Program {

	public static void main(String[] args) {

		double amount = 1000.00;
		String action = "";
		
		Bank bank = new Bank();

	    Account account1 = bank.addAccount("Jack", 1000.00);
	    Account account2 = bank.addAccount("Bob", 5000.00);
	    Account account3 = bank.addAccount("Bill", 20.00);
		// show accounts have been created
	    System.out.println(account1.getDetails());
	    System.out.println();
	    System.out.println(account2.getDetails());
	    System.out.println();
	    System.out.println(account3.getDetails());
	    
	    //look for account using owner name
	    Account foundAccount = bank.getAccount("Bob");
	    
	    if (foundAccount != null) {
	        System.out.println("Found account:");
	        System.out.println(foundAccount.getDetails());
	    }
	    
	    //try add an account past the allowed array	    
	    Account account4 = bank.addAccount("Jordan", 100.00);
	    
	    if (account4 == null) {
	        System.out.println("Bank is full. Could not add fourth account.");
	    }
	    
	    // check if account exists
	    
	    Account unknownAccount = bank.getAccount("David");
	    
	    if (unknownAccount == null) {
	        System.out.println("No account found for David.");
	    }
	    
		// array of accounts test
		//Account[] accounts = new Account[4];
		// accounts[0] = new Account(1, "Jack", 1000.00);
		// accounts[1] = new Account(2, "Bob", 5000.00);
		// accounts[2] = new Account(3, "Bill", 20.00);
		// accounts[3] = new Account(4, "Jordan", 100.00);
		//display array of users
		/*for(Account user : accounts) {
			// Display account details
			System.out.println("Account Details:");
			System.out.println(user.getDetails());
		}	*/
		
		/*/ account = account same thing
		accounts[3].AddInterest();		
		Account partnerAccount = accounts[3];		
		System.out.println(accounts[3].getDetails());		
		partnerAccount.AddInterest();		
		System.out.println(accounts[3].getDetails());*/		
		
		/*if (action == "Deposit") {
			accounts[0].deposit(amount);

		} else if (action == "Withdraw") {
			accounts[0].withdraw(amount);

		}else {
			
		}*/


	}
	
	static void processAmount(Account acc) {
		acc.addInterest();
		
	}

}
